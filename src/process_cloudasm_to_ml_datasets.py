...
def create_schema_fields(variables_dict):
    ...
    logging.warning(f"Schema fields created: {schema_fields}")
...
def upload_dataframe(bq_client, dataframe, table_id, schema):
    ...
    logging.warning(f"Uploading dataframe to {table_id}")
    job_config = bigquery.LoadJobConfig(schema=schema)
    job = bq_client.load_table_from_dataframe(
        dataframe, table_id, job_config=job_config
    )
    result = job.result()  # Wait for the job to complete
    logging.warning(f"Load job result for {table_id}: {result}")
...
def main():
    ...
    logging.warning(f"Config file : {config}")
    ...
    logging.warning(f"File names: {file_name}")
    logging.warning(f"Number of rows in raw dataframe: {len(df_raw)}")

    logging.warning("Create kernel functions")
    ...
    logging.warning("Remove unwanted chromosomes")
    df_filtered = filter_chromosomes(df_raw)
    logging.warning(f"Number of rows of df_filtered: {len(df_filtered)}")
    logging.warning(compute_counts_and_percentages(df_filtered["asm_snp"]))

    logging.warning("Calculate consecutive distances between CpGs")
    df_filtered["cpg_dist"] = df_filtered["cpg_pos"].apply(calculate_cpg_distances)

    logging.warning("Convert specific arrays into kernel densities")
    ...
    logging.warning(f"{percentage_removed:.2f}% of the rows were removed")
    ...
    logging.warning("One-hot encode categoricals variables that are not binary")
    dummies_list = []
    for var in categorical_vars_ohe:
        logging.warning(f"One hot for variable {var}")
    ...
    logging.warning("Enforcing data types for integer variables")
    for var in [
        "chr",
        "region_inf",
        "region_sup",
    ...
    logging.warning("Creating the dataset with a methylation sequence")
    dic_data["sequence_cpg_fm"] = dic_data["clean"][
        vars_to_keep + ["sequence_cpg_fm"]
    ].copy(deep=True)

    logging.warning("Creating the  dataset with the methylation matrix")
    dic_data["sequence_cpg_cov_and_methyl"] = dic_data["clean"][
        vars_to_keep + ["sequence_cpg_cov_and_methyl"]
    ].copy(deep=True)

    logging.warning("Creating the tabular dataset")
    dic_data["tabular"] = (
        dic_data["clean"]
    ...
    logging.warning(f"All variables in tabular dataset: {dic_data['tabular'].columns}")

    logging.warning("Uploading the 3 datasets to BigQuery")
    ...
    logging.warning("Uploading tabular data with schema autodetection")
    job_config = bigquery.LoadJobConfig(autodetect=True)
    job = bq_client.load_table_from_dataframe(
        dic_data["tabular"], f"{ml_dataset_id}.tabular", job_config=job_config
    )
    logging.warning(job.result())

    logging.warning("SCRIPT COMPLETE")
